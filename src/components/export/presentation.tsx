...
function PresentationRules({ rules, kind }: { rules: Rule[]; kind: 'linea_roja' | 'protocolo_incidencia' | 'instruccion_ia' }) {
  ...
  const kindConfig = {
    linea_roja: {
      ...
      bgClass: 'bg-[#85bf57]/5',
      icon: <Ban className="h-5 w-5 text-[#85bf57]" />
    },
    protocolo_incidencia: {
      colorClass: 'text-[#85bf57]',
      borderClass: 'border-[#85bf57]/20',
      bgClass: 'bg-[#85bf57]/5',
      icon: <ShieldAlert className="h-5 w-5 text-[#85bf57]" />
    },
    instruccion_ia: {
      colorClass: 'text-[#85bf57]',
      borderClass: 'border-[#85bf57]/20',
      bgClass: 'bg-[#85bf57]/5',
      icon: <BookOpen className="h-5 w-5 text-[#85bf57]" />
    }
  };
          ...
          const textBlock = (
            <div className="flex-1 space-y-3">
              <h2 className="text-lg font-bold tracking-tight text-white flex items-center gap-2">
                <span className="flex h-7 w-7 items-center justify-center rounded-full bg-sky-500/20 text-sm font-bold text-sky-400 shrink-0">
                  {idx + 1}
                </span>
                {mod.title}
              </h2>
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <div className="flex-1 overflow-y-auto px-16 py-12 flex justify-center items-start">
          ...
          <div className="pr-2">
            ...
            ) : blockDef.id === 11 || blockDef.id === 12 || blockDef.id === 13 ? (
              <div className="space-y-6 w-full">
                {content.trim() && (
                  <div className="markdown-preview max-w-none text-left">
                    <ReactMarkdown
                      components={{
                        a: ({ href, children, ...props }) => {
                          if (href === '#marker-pendiente') {
                ...
                <PresentationRules 
                  rules={rules} 
                  kind={blockDef.id === 11 ? "linea_roja" : blockDef.id === 12 ? "protocolo_incidencia" : "instruccion_ia"} 
                />
              </div>
            ) : content.trim() ? (
