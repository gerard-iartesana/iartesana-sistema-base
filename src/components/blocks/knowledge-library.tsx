...
export function KnowledgeLibrary({ brandId, onUpdate, readOnly = false }: KnowledgeLibraryProps) {
            ...
            return (
              <div key={item.id} className="transition-colors hover:bg-slate-50/30">
                ...
                <div className="flex items-center gap-3 px-4 py-2.5">
                  ...
                  <button
                    onClick={() => !readOnly && handleToggleVerified(item.id, item.verified)}
                    disabled={readOnly}
                    className={`rounded p-1 transition-colors ${readOnly ? 'text-slate-350 cursor-default' : (item.verified ? 'text-emerald-500 hover:bg-emerald-50' : 'text-slate-300 hover:bg-slate-100 hover:text-slate-500')}`}
                  ...
                  <button
                    ...
                    title={readOnly ? 'Ver detalle' : 'Editar ítem'}
                  >
                ...
                {isExpanded && (
                  readOnly ? (
                    <div className="border-t border-[#2a2a2f] bg-slate-50 p-4">
                      <div className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider mb-2 select-none">Contenido del ítem</div>
                      <div className="text-xs text-slate-700 bg-white border border-slate-200 rounded-lg p-3 whitespace-pre-wrap leading-relaxed font-mono">
                        {item.body_md || 'Sin contenido'}
                      </div>
                    </div>
                  ) : (
